...
public class GestorDocumentalServiceImpl implements GestorDocumentalService {
	...
	// TODO Remv
	@Autowired
	private EmptySignature emptySignature;
...
//							}
							...
							try {
								...
								Registro registro = new Registro(expediente, identificadorExpediente, nombreDocumento,
										null, "Error al cerrar el documento: " + e.getMessage(), "ERROR");
								registroService.insert(registro);
								throw new GeneralException(e);
							}
						}
...
//				}
	...
	private RespuestaGenerica cerrarDocumento(String idDocumento) throws GeneralException {
		...
		String urlParte = MessageFormat.format(gedeApiDocumentosCerrar, idDocumento);
		ResponseEntity<RespuestaGenerica> response = restTemplate.exchange(urlParte, HttpMethod.PUT, requestEntity,
				new ParameterizedTypeReference<RespuestaGenerica>() {
				});
		if (HttpStatus.CREATED.equals(response.getStatusCode())) {
	 ...
	 * @throws GeneralException the general exception
	 */
	private RespuestaGenerica establecerFirmaDocumento(String idDocumento, String idBinarioFirma)
			throws GeneralException {
