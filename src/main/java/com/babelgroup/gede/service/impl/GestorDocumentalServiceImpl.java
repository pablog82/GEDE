                ...
                for (File ficheroExpediente : listadoFicherosExpediente) {
                    ...
                    if (registros.isEmpty()) {
                        ...
                        if (firmar) {
                            ...
                            File fileFirmaDocumento = new File(ficheroExpediente.getAbsolutePath().toUpperCase().replace(".PDF",
                            ...
                            try {
                                ...
                                RespuestaGenerica firmaResponse = establecerFirmaDocumento(nombreDocumento, null);
                            } catch (GeneralException e) {
                                Registro registro = new Registro(expediente, identificadorExpediente, nombreDocumento,
...
//                        registroService.insert(registro);
    ...
    private void obtenerTicket() throws GeneralException {
        ...
        if (HttpStatus.CREATED.equals(response.getStatusCode())) {
            ...
            log.info("obtenerTicket() - Ticket obtenido:  " + this.ticketResponse.getTicket());
    ...
    private RespuestaGenerica almacenarBinario(File fichero) throws GeneralException {
        ...
        try {
            ...
            if (HttpStatus.CREATED.equals(response.getStatusCode())) {
                ...
                log.info("almacenarBinario() - Binario almacenado:  " + r.getIdentificador());
                return r;
...
//				}
     ...
     *
     * @param idDocumento the id documento
     * @param idBinarioFirma   the metadatos
     * @return the firma response
     * @throws GeneralException the general exception
     */
    private RespuestaGenerica establecerFirmaDocumento(String idDocumento, String idBinarioFirma) throws GeneralException {
        ...
        String urlParte = MessageFormat.format(gedeApiDocumentosEstablecerFirma, idDocumento, idBinarioFirma);

        ResponseEntity<RespuestaGenerica> response = restTemplate.exchange(urlParte,
                HttpMethod.PUT, requestEntity, new ParameterizedTypeReference<RespuestaGenerica>() {
                });
        if (HttpStatus.CREATED.equals(response.getStatusCode())) {
            return response.getBody();
        } else if (HttpStatus.UNAUTHORIZED.equals(response.getStatusCode()) && intentosLlamadaAPI < 3) {
            ...
            return establecerFirmaDocumento(idDocumento, idBinarioFirma);
        } else {
